...
def parsedocs(file1, file2):
    ...
    get_containment_matrices(root1, root2)
...
def get_containment_matrices(node1, node2):
    countChild1 = degree2(node1, "")
    countChild2 = degree2(node2, "")
    # for child1 in node1:
    #     for child2 in node2:
    ...
    if n == 0 and m == 0:
        ...
        for i in range(1, n + 1):
            for j in range(1, m + 1):
                ...
                z = nier_jaga(temp0[i], temp1[j])
                if z == 0:
                    EDistance[i - 1, j]
                updating = EDistance[i - 1, j - 1] + z # cost of updating
